...
@section('content')
    <div class="space-y-6">
        <div>
            <h1 class="text-base-content text-3xl font-bold">Create Permission</h1>
            <p class="text-base-content/70 mt-1">Add a new permission</p>
        </div>
        ...
        <div class="bg-base-100 shadow-base-300/20 w-full space-y-6 rounded-xl p-6 shadow-md lg:p-8">
            <form action="{{ route('admin.permissions.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" placeholder="Enter permission name" class="input @error('name') input-error @enderror" id="name" value="{{ old('name') }}" required />
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" name="slug" placeholder="permission-slug (auto-generated if empty)" class="input @error('slug') input-error @enderror" id="slug" value="{{ old('slug') }}" />
                    @error('slug')
                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <select name="type" id="type" class="select @error('type') select-error @enderror" required onchange="togglePermissionTargets()">
                        ...
                        <option value="submodule" {{ old('type') == 'submodule' ? 'selected' : '' }}>Submodule</option>
                    </select>
                    @error('type')
                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                    @enderror
                ...
                <div id="submodule-field" style="display: none;">
                    ...
                    <select name="submodule_id" id="submodule_id" class="select @error('submodule_id') select-error @enderror">
                        ...
                        @foreach ($modules as $module)
                            @foreach ($module->submodules as $submodule)
                                <option value="{{ $submodule->id }}" data-module="{{ $module->id }}" {{ old('submodule_id') == $submodule->id ? 'selected' : '' }}>{{ $module->name }} - {{ $submodule->name }}</option>
                            @endforeach
                        @endforeach
                    </select>
                    @error('submodule_id')
                ...
                <div>
                    ...
                    <textarea name="description" placeholder="Enter permission description" class="textarea @error('description') textarea-error @enderror" id="description" rows="3">{{ old('description') }}</textarea>
                    @error('description')
                    <span class="text-error text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
